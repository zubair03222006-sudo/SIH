...
const earthFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Full crystal clear daytime map everywhere (no dark night side)
    vec3 dayCol = texture2D(dayMap, vUv).rgb;

    // Vibrant crystal clear lighting with subtle directional contrast
    vec3 color = dayCol * 1.25;

    // Crisp ocean specular shine
    float water = texture2D(specMap, vUv).r;
    vec3 viewDir = normalize(cameraPosition - vWorldPos);
    vec3 halfDir = normalize(viewDir + sunDir);
    float spec = pow(max(dot(worldN, halfDir), 0.0), 30.0) * water;
    color += vec3(0.5, 0.75, 1.0) * spec * 0.4;

    // Subtle topo elevation details
    float topo = texture2D(topoMap, vUv).r;
    color *= 0.92 + topo * 0.18;
...
const atmosphereFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec3 col = glowColor;
    float alpha = fres * intensity * 0.9;
...
export function Earth({ radius = 1, sunDirection }: { radius?: number; sunDirection: THREE.Vector3 }) {
  ...
  const atmoUniforms = useMemo(
    () => ({
      glowColor: { value: new THREE.Color("#3a7ac8") },
