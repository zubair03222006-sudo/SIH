...
export interface EventsState {
  ...
  error: string | null;
  addEvent: (
    evt: Partial<LiveEvent> & { title: string; coords: [number, number]; hazardType: HazardType },
  ) => LiveEvent;
...
export function EventsProvider({ children }: { children: ReactNode }) {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      setError(
        evts.length === 0
          ? "No verified live alerts are currently available. Demo alerts are disabled."
          : null,
      );
    } catch (err) {
      console.error("Live alert fetch failed", err);
      setEvents([]);
      setError("Live alert service is unavailable. No demo alerts are being shown.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const addEvent = useCallback(
    (
      ...
      fetch(`${API_BASE}/api/events`, {
        ...
        .then((realEvt) => {
          setEvents((prev) => prev.map((e) => (e.id === tempId ? realEvt : e)));
        })
        .catch((err) => {
